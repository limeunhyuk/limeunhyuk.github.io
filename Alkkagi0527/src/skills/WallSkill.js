...
export class WallSkill extends BaseSkill {
    ...
    onActivate() {
        ...
        const previewColor = state.currentTurn === 'black' ? 0x44aaff : 0xffffff;
    ...
    placeWall() {
        ...
        const wallColor    = isBlack ? 0x223344 : 0xeeeeee;  // 흑: 짙은 청회색
        const emissiveCol  = isBlack ? 0x2288ff : 0xffffff;  // 흑: 파란빛 발광
        const emissiveInt  = isBlack ? 0.85     : 0.45;      // 흑: 강한 발광
        ...
        if (isBlack) {
            ...
            const edgesMat = new THREE.LineBasicMaterial({ color: 0x55ccff, linewidth: 2 });
